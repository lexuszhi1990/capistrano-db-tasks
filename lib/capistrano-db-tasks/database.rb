  ...
  private
    ...
    def import_cmd(file)
      if mysql?
        ...
        "#{pgpass} psql -c \"#{terminate_connection_sql}\" #{credentials} -d #{database}; #{pgpass} dropdb #{credentials} #{database}; #{pgpass} createdb #{credentials} #{database}; #{pgpass} psql #{credentials} -d #{database} < #{file}"
      end
    end
